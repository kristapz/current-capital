...
# =======================
def configure(project_id: str, dataset_id: str, source_table_id: str, destination_table_id: str):
    # Set your Google Cloud credentials
    os.environ['GOOGLE_APPLICATION_CREDENTIALS'] = 't'
    ...
    config['client'] = bigquery.Client(project=project_id)
...
def calculate_relative_stock_index(stock_hist: pd.DataFrame, benchmark_hist: pd.DataFrame, days: int = 3) -> Optional[float]:
...
# =======================
# 7. BigQuery Functions
# =======================

def insert_to_bigquery(client: bigquery.Client, dataset_id: str, destination_table_id: str, data: Dict):
...
def check_and_create_table(client: bigquery.Client, dataset_id: str, destination_table_id: str):
    """Checks if the destination table exists, and creates it if it does not."""
    try:
        table_ref = client.dataset(dataset_id).table(destination_table_id)
        client.get_table(table_ref)
        logger.info(f"Table {destination_table_id} already exists.")
    except NotFound:
        schema = [
        ...
        logger.info(f"Created table {destination_table_id}.")
...
async def fetch_stock_data(session: aiohttp.ClientSession, ticker: str, stock_cache: StockDataCache) -> Optional[Dict]:
...
async def process_and_insert(session: aiohttp.ClientSession, ticker: str, sp500_hist: pd.DataFrame, config: Dict, stock_cache: StockDataCache):
    """Processes a single stock and inserts it into BigQuery."""
    data = await fetch_stock_data(session, ticker, stock_cache)
    if data:
        processed = process_stock_data(ticker, data, sp500_hist)
        if processed:
            insert_to_bigquery(config['client'], config['dataset_id'], config['destination_table_id'], processed)

async def process_batch(session: aiohttp.ClientSession, tickers: List[str], sp500_hist: pd.DataFrame, config: Dict, stock_cache: StockDataCache):
    ...
    async def sem_task(ticker):
        async with semaphore:
            await process_and_insert(session, ticker, sp500_hist, config, stock_cache)
...
async def fetch_and_process_stocks_async(config: Dict):
    """Main async function to fetch and process stocks."""
    check_and_create_table(config['client'], config['dataset_id'], config['destination_table_id'])
    ...
    async with aiohttp.ClientSession() as session:
        ...
        while True:
            ...
            query = f"""
            SELECT ticker FROM `{config['source_full_table_id']}`
            ORDER BY ticker
            ...
            # Process the batch
            await process_batch(session, valid_tickers, sp500_hist, config, stock_cache)
...
def main(source_table_id: str):
    # Define project and dataset
    project_id = 'test1-427219'
    dataset_id = 'stock_datasets'

    # Generate destination table ID with current date
    today = datetime.now().strftime('%Y%m%d')  # e.g., 20250102
    destination_table_id = f"stocksenhanced_{today}"

    # Configure BigQuery settings
    config = configure(project_id, dataset_id, source_table_id, destination_table_id)

    asyncio.run(fetch_and_process_stocks_async(config))

if __name__ == "__main__":
    import sys
    if len(sys.argv) != 2:
        print("Usage: python stock_processor.py <source_table_id>")
        sys.exit(1)
